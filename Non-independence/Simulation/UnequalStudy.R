...
rep.low = 3
rep.up = 20
#Number of iteration in the simulation#
iteration = 5000

#Standard deviation of the random study effects#
sd.e = c(0.1, 0.5, 1)
#Because correlation is unequal for each pair, we specify the range of correlation for each paper#
#The range is rho.e to rho.e+rho.e.range#
rho.e = c(0.1, 0.6)
rho.e.range = 0.4
...
parm$mu = rep(c(0.5,4.5,14.5), times=6)
scenario = dim(parm)[1]
...
for(k in 1:scenario) {
	...
	raw.data = data.frame()
	for(i in 1:no.paper){ #Loop though each paper#
	...
	#Two step method using one randomly chosen study per paper#
...
save.image(file="UnequalStudy.RData")