...
boot.ci(boot.p, type="norm")
boot.p.paper = boot(data=p.paper.all, statistic=SigPropCalc, R=4000)
boot.ci(boot.p.paper, type="norm")

#Contrast MA with or without accounting for non-independence#
accounted = c("272.R", "282.R", "585.R", "303.R", "759.R", "952.R", "628.R", "424.R")
not.accounted = c("53.R", "2.R", "82.R", "120.R", "511.R", "695.R", "1365.R", "1307.R", "1180.R", "1185.R", "674.R", "639.R")
...
for(i in 1:9){
	hist(p.all[i,i.not.accounted], breaks=seq(0,1,0.05), col=rgb(0,0,0,0.4), border=NA, main="", xlab="p-value", ylim=c(0,8))
	hist(p.all[i,i.accounted], breaks=seq(0,1,0.05), col=rgb(0,0.5,1,0.4), border=NA, add=T)
	if(i==1){legend("topleft", legend=c("Account for","Not account for"), fill=c("grey","dodgerblue"), bty="n", border=NA) }
}
...
legend("topright", legend=c("Account for","Not account for"), fill=c("dodgerblue","grey"), bty="n", border=NA)
...
quartz(w=8,h=4)
par(mfrow=c(1,2), mar=c(4,4,1,1))
plot(p.original.paper.all~p.original.all, xlim=c(0,1), ylim=c(0,1), xlab="Original p-values", ylab="P-values with a paper effect")
abline(0,1, lty=2, col="grey")
abline(h=0.05, lty=2, col="grey")
abline(v=0.05, lty=2, col="grey")
plot(p.original.paper.all~p.original.all, xlim=c(0,0.05), ylim=c(0,0.05), xlab="Original p-values", ylab="P-values with a paper effect")
abline(0,1, lty=2, col="grey")
...
quartz(w=4.5,h=4.5)
par(mfrow=c(1,1), mar=c(4,4,1,1), cex=0.85)
...
legend("topright", legend=c("Not account for","Account for"), pch=19, col=c("grey","dodgerblue"), bty="n")
