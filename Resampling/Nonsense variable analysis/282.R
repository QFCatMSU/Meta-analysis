...
mod.ns1 = rma.mv(yi ~ year.parity + Pathway * Type.of.interaction, V = V[x,x], random = ~factor(Case.ID)|factor(Study.ID), data=  droplevels(es[x,]), btt=2)
mod.ns2 = rma.mv(yi ~ name.parity + Pathway * Type.of.interaction, V = V[x,x], random = ~factor(Case.ID)|factor(Study.ID), data=  droplevels(es[x,]), btt=2)
mod.ns3 = rma.mv(yi ~ name.rank + Pathway * Type.of.interaction, V = V[x,x], random = ~factor(Case.ID)|factor(Study.ID), data=  droplevels(es[x,]), btt=2)
mod.ns4 = rma.mv(yi ~ name.vowels + Pathway * Type.of.interaction, V = V[x,x], random = ~factor(Case.ID)|factor(Study.ID), data=  droplevels(es[x,]), btt=2)
