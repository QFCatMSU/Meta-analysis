...
parm = expand.grid(sd.e = sd.e, mu.log.sigma = mu.log.sigma, var.log.sigma = var.log.sigma, mu.rep = mu.rep, missing = missing)
...
result <- foreach(k = 1:scenario) %dopar% {
	...
	for(i in 1:iteration){
		...
		parm.est[i, 2] <- as.numeric(try(fixef(mod2), silent = T))
...
save.image(file = "missing_hierarchical_unweighted.RData")