...
result <- foreach(k = 1:scenario) %dopar% {
	...
	for(i in 1:iteration){
		...
		if(parm$missing[k] == 0){
			mod <- try(rma.mv(yi = ES, V = ES.var, random = list(~1|group, ~1|study), 
		...
		if(parm$missing[k] != 0){
			...
			mean.rep.cv <- mean(effect.size.reduced$rep.CV2)
			effect.size.imputed <- 
			  effect.size %>%
			  mutate(ES.var = replace(x = ES.var, list = which.miss, value = mean.rep.cv / no.rep[which.miss]))
		
		    #Weighted analysis#
		    mod1 <- try(rma.mv(yi = ES, V = ES.var, random = list(~1|group, ~1|study), 
		                       test = "t", data = effect.size.reduced), silent = T)
		    parm.est[i, 1] <- try(mod1$b, silent = T) %>% as.numeric()
		
		    #Imputation using mean among replicate CV#
		    mod2 <- try(rma.mv(yi = ES, V = ES.var, random = list(~1|group, ~1|study), 
		                test = "t", data = effect.size.imputed), silent = T)
...
save.image(file = "missing_hierarchical_impute_CV.RData")