...
result = foreach(k = 1:2400) %dopar% {
	...
	for(i in 1:iteration){
		...
		group.eff = rep(rnorm(n=10, mean=0, sd=parm$sd.e[k]/sqrt(4)*sqrt(2)), each=5)
		for(m in 1:no.study){
			log.e = rnorm(n=1, mean=0, sd=parm$sd.e[k]/sqrt(4)*sqrt(2))
		...
		mod1 = try(rma.mv(yi=ES, V=ES.var, random=list(~1|group,~1|study), test="t", data=effect.size), silent=T)
		mod2 = try(rma.mv(yi=ES, V=ES.var.true, random=list(~1|group,~1|study), test="t", data=effect.size), silent=T)	
		parm.est[i,1] = as.numeric(try(mod1$b, silent=T))
		parm.est[i,2] = as.numeric(try(mod2$b, silent=T))
		mod3 = try(lme(ES~1, random=~1|group, data=effect.size), silent=T)
		parm.est[i,3] = as.numeric(try(fixef(mod3), silent=T))
...
save.image(file="VarSigma_I2_HIER.RData")
